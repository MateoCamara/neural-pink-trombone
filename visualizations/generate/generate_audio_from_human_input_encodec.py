...
if __name__ == '__main__':
    ...
    all_available_configs = [config for config in all_available_configs if ("encodec" in config) and ("dynamic" in config)]
    ...
    for config_path in all_available_configs:
        ...
        for audio_file_name in tqdm(os.listdir(human_audios_path)):
            ...
            audio_tensor = torch.as_tensor(audio_file).float().unsqueeze(0).unsqueeze(0)  # Add batch dimension
            ...
            embbeding_interpolated = torch.tensor(interpolate_embeddings(emb, original_fps=emb.shape[0], target_fps=int(np.round(audio_file.shape[0]/512)))).float()
